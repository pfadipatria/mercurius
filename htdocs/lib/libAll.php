<?php

require_once 'libTranslate.php';
require_once 'libFrontend.php';
require_once 'libUser.php';
require_once 'libDb.php';
require_once 'libKey.php';
require_once 'libLock.php';
require_once 'libPerson.php';
require_once 'libRouting.php';
require_once 'libRender.php';
require_once 'libConfig.php';
require_once 'libHistory.php';
require_once 'libHelp.php';
require_once 'libSearch.php';
require_once __DIR__.'/../src/SKeyManager/Repository/AbstractRepository.php';
require_once __DIR__.'/../src/SKeyManager/Repository/PersonRepository.php';
require_once __DIR__.'/../src/SKeyManager/Repository/KeyRepository.php';
require_once __DIR__.'/../src/SKeyManager/Repository/LockRepository.php';
require_once __DIR__.'/../src/SKeyManager/Repository/PermissionRepository.php';
require_once __DIR__.'/../src/SKeyManager/Repository/HistoryRepository.php';
require_once __DIR__.'/../src/SKeyManager/Repository/SearchRepository.php';
require_once __DIR__.'/../src/SKeyManager/Entity/AbstractEntity.php';
require_once __DIR__.'/../src/SKeyManager/Entity/Person.php';
require_once __DIR__.'/../src/SKeyManager/Entity/Key.php';
require_once __DIR__.'/../src/SKeyManager/Entity/Lock.php';
require_once __DIR__.'/../src/SKeyManager/Entity/Permission.php';
require_once __DIR__.'/../src/SKeyManager/Entity/History.php';
